// Live harness e2e test: AgentHarness + real API (oc/deepseek-v4-flash-free)
// Measures node process CPU via process.cpuUsage() to check 0.1-core budget.
...
import { AgentHarness } from "../packages/agent/src/harness/agent-harness.ts";
import { NodeExecutionEnv } from "../packages/agent/src/harness/env/nodejs.ts";
import { JsonlSessionStorage } from "../packages/agent/src/harness/session/jsonl-storage.ts";
import { Session } from "../packages/agent/src/harness/session/session.ts";
...
const provider = createProvider({
	...
	baseUrl: API,
	auth: { apiKey: { name: "router key", login: async () => Object.assign({ type: "api_key" }, { key: KEY }), resolve: async () => ({ auth: { apiKey: KEY }, source: "hardcoded" }) } },
	models: [{
		id: MODEL_ID,
...
try {
	...
	const storage = await JsonlSessionStorage.create(env, join(dir, "session.jsonl"), { cwd: dir, sessionId: "live-oc-1" });
	...
	console.log(`reply role: ${reply.role}, content: ${JSON.stringify(reply.content?.slice?.(0, 120) ?? reply.content)}`);
