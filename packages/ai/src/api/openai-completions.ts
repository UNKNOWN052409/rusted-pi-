...
function createClient(
	...
	// Sryze-style proxies stream raw JSON bodies instead of SSE and their WAF
	// blocks the default openai-node user-agent (403 Your request was blocked).
	// Rewrite the UA and synthesize SSE chunks from the raw JSON body.
	if (compat.streamsRawJson) {
		headers["user-agent"] = "openai-node/4.x";
	}
	...
	const effectiveFetch = compat.streamsRawJson ? createSseConvertingFetch() : fetch;
...
/**
 * Fetch wrapper for proxies that return a single raw JSON completion body
 * instead of an SSE stream. Reads the full body, converts it to synthetic
 * `chat.completion.chunk` SSE events (matching the shape the SDK's SSE
 * parser expects), and returns a `text/event-stream` Response.
 */
function createSseConvertingFetch(): typeof globalThis.fetch {
	return async (input, init) => {
		const headers = new Headers(init?.headers);
		headers.set("user-agent", "openai-node/4.x");
		const res = await fetch(input, { ...init, headers });
		const contentType = res.headers.get("content-type") ?? "";
		if (!res.ok || contentType.includes("text/event-stream") || contentType.includes("application/json") === false) {
			return res;
		}
		const text = await res.text().catch(() => "");
		const cleaned = text.replace(/\s*data: \[DONE\]\s*$/, "").trim();
		let completion: unknown;
		try {
			completion = JSON.parse(cleaned);
		} catch {
			// Not raw JSON (e.g. an HTML error page) — pass through untouched.
			return new Response(cleaned, { status: res.status, headers: res.headers });
		}
		const sse = (obj: unknown) => `data: ${JSON.stringify(obj)}\n\n`;
		const comp = completion as {
			id?: string;
			created?: number;
			model?: string;
		...
		const base = {
			id: comp.id ?? "chatcmpl-proxy",
			object: "chat.completion.chunk",
			created: comp.created ?? Math.floor(Date.now() / 1000),
			model: comp.model ?? "",
		};
		const chunks: string[] = [];
		chunks.push(sse({ ...base, choices: [{ index: 0, delta: { role: "assistant" }, finish_reason: null }] }));
		if (msg?.content) {
			chunks.push(sse({ ...base, choices: [{ index: 0, delta: { content: msg.content }, finish_reason: null }] }));
		}
		chunks.push(sse({ ...base, choices: [{ index: 0, delta: {}, finish_reason: finish }] }));
		if (comp.usage) {
			chunks.push(sse({ ...base, choices: [], usage: comp.usage }));
		}
...
function detectCompat(model: Model<"openai-completions">): ResolvedOpenAICompletionsCompat {
	...
	// Sryze-style OpenAI-compatible proxies: WAF blocks the openai-node SDK user-agent
	// (403 Your request was blocked.) and stream non-SSE raw JSON bodies. Detect by
	// provider name or host so we can rewrite the UA and synthesize SSE.
	const isSryzeProxy = provider === "sryze" || baseUrl.includes("sryze.cc") || baseUrl.includes("ourproxy.");
...
function getCompat(model: Model<"openai-completions">): ResolvedOpenAICompletionsCompat {
	...
	return {
		...
		streamsRawJson: model.compat.streamsRawJson ?? detected.streamsRawJson,
	};
}
