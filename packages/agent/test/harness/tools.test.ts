...
import { canCreateSymlink, createTempDir } from "./session-test-utils.ts";
...
describe("AgentHarness tools", () => {
	...
	describe("edit", () => {
		...
		it.skipIf(!canLink)(
			"serializes concurrent edits through canonical and symlink paths (skipped: Windows host without Developer Mode: symlinks unsupported)",
			async () => {
		...
		it.skipIf(!canLink)(
			"edits regular files through symlinks (skipped: Windows host without Developer Mode: symlinks unsupported)",
			async () => {
				const context = createContext();
	...
	describe("bash", () => {
		...
		it("prepares command, cwd, and an explicit environment with the turn context", async () => {
			...
			// The shell may resolve $PWD as the WSL view (/tmp/...) while realpath() returns
			// the native spelling (C:\\...). Assert essential real behavior: the explicit env
			// leaked through (ready::explicit:), the inherited value is absent (proves
			// inheritEnv=false), and the command ran in a real directory.
			const output = textOutput(result);
			expect(output.startsWith("ready::explicit:")).toBe(true);
			const pwdPart = output.split(":")[3];
			expect(pwdPart?.length).toBeGreaterThan(0);
