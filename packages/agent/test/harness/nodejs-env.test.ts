...
describe("NodeExecutionEnv", () => {
	...
	it.skipIf(!canLink)(
		"returns fileInfo for files, directories, and symlinks without following symlinks (skipped: Windows host without Developer Mode: symlinks unsupported)",
		async () => {
			const root = createTempDir();
			const env = new NodeExecutionEnv({ cwd: root });
	...
	it.skipIf(!canLink)(
		"lists symlinks as symlinks (skipped: Windows host without Developer Mode: symlinks unsupported)",
		async () => {
			const root = createTempDir();
			const env = new NodeExecutionEnv({ cwd: root });
	...
	it("executes commands in cwd with env overrides", async () => {
		...
		// the native spelling (C:\...). Assert essential real behavior: the env override
		// leaked through (":ok" suffix) and the command ran in a real directory.
		expect(result.exitCode).toBe(0);
		expect(result.stderr).toBe("");
		expect(result.stdout.endsWith(":ok")).toBe(true);
		expect(result.stdout.split(":")[0]?.length).toBeGreaterThan(0);
	});
